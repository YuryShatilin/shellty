...
public class CodeGen {
    ...
    private String declarationKeyword = "declare";

    public void setExternalDeclaration() {
        declarationKeyword = "declare";
    }

    public void setLocalDeclaration() {
    ...
    public void insertVarDeclaration(Node varNode) {
        ...
        if (varNode.getData().isArrayVar()) {
            insertLine(declarationKeyword + " -A " + varName);
            return;    
        }
        ...
        case STRING:
            if (!varNode.getData().getValue().isEmpty()) {
                insertLine(String.format(declarationKeyword + " %s=%s", varName, 
                            varNode.getData().getValue()));
            } else {
                insertLine(String.format(declarationKeyword + " %s", varName));
            }
            return;
        case INTEGER:
            if (!varNode.getData().getValue().isEmpty()) {
                insertLine(String.format(declarationKeyword + " -i %s=%s", varName, 
                            varNode.getData().getValue()));
            } else {
                insertLine(String.format(declarationKeyword + " -i %s", varName));
            }
            return;
        default:
            return;
